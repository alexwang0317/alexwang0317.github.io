...
<article class="layout-md py-12 mx-auto max-w-3xl px-6 svelte-1tg8pcv"><div class="mb-8"><a href="/python" class="text-blue-600 hover:underline text-sm">← Back to Python</a></div>

  <h1 class="text-3xl font-bold mb-4">HPP Blog #1: Python Profiling</h1>

  <div class="text-sm text-neutral-500 mb-8">September 5, 2025 • 15 min read
  </div>

  <div class="text-lg text-neutral-700 mb-8 leading-relaxed italic">&quot;Premature optimization is the root of all evil&quot; - Donald Knuth
  </div>

  <div class="prose prose-neutral max-w-none"><p>In this post, which will be the first in a series, I will go over the
    ...
    <p>At the moment, these are the reasons why I personally think profiling is
      becoming more important:
    </p>

    <ul><li><strong>Don&#39;t over-optimize the wrong thing.</strong> Most problems have
      ...
      <li><strong>Fix what you believe is the problem.</strong> Oftentimes, you skip
      ...
      <li><strong>Rise of vibe coding.</strong> Vibe coding/AI coding is not inherently
    ...
    <p>As a heads-up, the implementation that is used here is deliberately the
    ...
    <p>The simplest way to profile would most definitely be to just use time and
      print statements. Print statements are already super useful in debugging,
      so why not profiling as well?
    </p>

    <p>The simplest way to go about using time and print statements is simply
...
def timePrintFn(fn):
    ...
    <p>Of note, on other texts, such as the textbook High Performance Python,
    ...
    <p>Another general method for doing this is using the <code>%timeit</code>
...
%timeit -n N -r R multiply_matrices_pure_python(matrix_a, matrix_b)</code></pre>
    ...
    <p>You can also try <code>%%time</code>, which basically times an entire cell
      in a jupyter notebook.
    </p>

    <p>First, when measuring functions this way, it&#39;s helpful to remove the first
    ...
    <p>cProfile is the faster of the two profilers in the standard Python library
      because it&#39;s written in C.
    </p>

    <p>Before you profile anything, you should probably first make a guess, or
    ...
    <p>One of the ways to use cProfile is as a command-line argument. The CLI
    ...
    <p>The one that I think is most useful is to use the cumulative flag, which
    ...
    <p>If you are in the repo, and run it on the matmul.py script, you will get
      something like this:
    </p>
...
[2465470, 2417773, 2460009]
  ...
  2000000    0.066    0.000    0.066    0.000 ${escape("{")}method &#39;bit_length&#39; of &#39;int&#39; objects${escape("}")}
    ...
    <p>If you compare, we&#39;ve about doubled our time (this new one took ~120
    ...
    <ul><li><strong>ncalls:</strong> total number of calls. The first number is the total
        number of calls, while the second number is the primitive calls––basically
        any call that was not recursive
      </li>
      <li><strong>tottime:</strong> time spent in that function excluding the function
        call itself
      </li>
      <li><strong>percall:</strong> tottime/ncalls</li>
      <li><strong>cumtime:</strong> across all sub-functions called included
      </li>
      <li><strong>percall:</strong> cumtime/ncalls</li></ul>

    <p>Looking at the above code, it&#39;s clear that the heavy lifting is being done
    ...
    <p>Then to get further analysis, we can run the following to save it into a
      statistics folder:
    </p>

    <pre><code class="language-bash">$ python -m cProfile -o matmul.stats matmul.py</code></pre>

    <p>After doing this, feel free to check out the jupyter notebook on
      profiling. Then afterwards you can see that we can call <code>print_callers()</code>
    ...
    <p>SnakeViz is a way to draw the output of cProfile as flame graph. The
    ...
    <p>Then run <code>!snakeviz</code> in jupyter notebook or then calling it in CLI
...
$ snakeviz matmul.stat</code></pre>
    ...
    <p>You can then sort by various strategies like cumtime, percall, or ncalls.
    </p>
    ...
    <p>Line profiler is more granular method of profiling functions, and works by
    ...
    <p>Then mark the function you want to profile with the decorator <code>@profile</code>, then run it with the following command (details
    ...
    <p>Of note, this takes significantly more time, so you may want to edit the
...
==============================================================
    ...
    <ul><li><strong>Line #:</strong> The line number in the source file</li>
      <li><strong>Hits:</strong> Basically how many times this particular line was
        run
      </li>
      <li><strong>Time:</strong> Amount of time units is an arbitrary measurement (different
      ...
      <li><strong>Per Hit:</strong> Time / Hits, basically how many time units per
        hit took on average.
      </li>
      <li><strong>% Time:</strong> what percentage of the total time was spent in this
        line.
      </li></ul>

    <p>Afterwards, if you want even more granular flow, for different programs,
    ...
    <p>Memory usage helps you 1. Minimize RAM usage and 2. Optimize RAM vs. CPU
    ...
    <p>You use the same <code>@profile</code> header as before. So simply call:
    </p>
...
=============================================================
    ...
    <p>However, we can make a RAM memory for speed tradeoff by storing the rows
      and the columns so the number of calls for each row is not significantly
      more.
    </p>

    <p>This new implementation with caching the rows of the first matrix and the
...
=============================================================
    ...
    48   53.766 MiB    0.000 MiB           1       return C</code></pre></div>

  <div class="border-t pt-8 mt-12"><p class="text-sm text-neutral-600 mb-4">Thanks to <a href="https://www.dkzhang.com/" class="text-blue-600 hover:underline">David Zhang</a> for the helpful comments.
    </p>
    <p class="text-sm text-neutral-500">Questions or feedback? Feel free to reach out!
    </p></div>
</article>`;
});
export {
